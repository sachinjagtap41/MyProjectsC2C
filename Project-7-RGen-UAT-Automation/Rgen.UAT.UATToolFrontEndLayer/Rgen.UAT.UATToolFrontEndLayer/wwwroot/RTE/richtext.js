...
function writeRichText(rte, html, width, height, buttons, readOnly) {
	if (isRichText) {
		...
		document.writeln('<iframe width="154" height="104" id="cp' + rte + '" src="' + includesPath + 'palette.aspx" marginwidth="0" marginheight="0" scrolling="no" style="visibility:hidden; position: absolute;margin-top:150px"></iframe>');
...
function rteCommand(rte, command, option) {
	...
	try {
		oRTE.focus();
		oRTE.document.execCommand(command, true , option);
	    oRTE.focus();
...
function dlgInsertTable(rte, command) {
	...
	window.open(includesPath + 'insert_table.aspx', 'InsertTable', windowOptions);
}

function insertLink(rte) {
	//function to insert link
	
	//Commented by Ankita to show Insert link option window only once: 7/8/2012
	//var szURL = prompt("Enter a URL:", "");
	
	
	try {
		//ignore error for blank urls
		//Commented by pallavi on 11 july
		
		//rteCommand(rte, "Unlink", null);
		rteCommand(rte,'CreateLink',null);
		
		//commented by Ankita as var szURL was commented : 7/8/2012
...
function selectFont(rte, selectname,OptionValue)
 {

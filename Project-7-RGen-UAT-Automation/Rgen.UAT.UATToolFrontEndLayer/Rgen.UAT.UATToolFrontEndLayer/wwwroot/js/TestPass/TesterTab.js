...
/**************************************************/
	...
	currProjectIndex:'',
  	...
  	/****************Added by Mohini for Resource file***********/
  		if(resource.isConfig.toLowerCase()=='true')
	    {
          ...
          $("#groupHead label").html(tester.gConfigGroup+'<img src="../SiteAssets/images/drop-arrow.gif" style="float:right;padding-top:5px"/>');
          $("#portfolioHead label").html(tester.gConfigPortfolio+'<img src="../SiteAssets/images/drop-arrow.gif" style="float:right;padding-top:5px"/>');
          $("#projectHead label").html(tester.gConfigProject+'<img src="../SiteAssets/images/drop-arrow.gif" style="float:right;padding-top:5px"/>');
          $("#versionHead label").html(tester.gConfigVersion+'<img src="../SiteAssets/images/drop-arrow.gif" style="float:right;padding-top:5px"/>');
		  $("#TestPassHead label").html(tester.gConfigTestPass+'<img src="../SiteAssets/images/drop-arrow.gif" style="float:right;padding-top:5px"/>');
	...
	populateArea:function()
	{
	    ...
	    var msg = ServiceLayer.GetData("GetArea","");
	...
	getTesters:function()
	{
		tester.TesterResult = ServiceLayer.GetData("GetTestersByTestPassID",show.testPassId);
	},
	...
	updateRoles:function()
	{
	   ...
	   /*************************************/					      
		...
		else
		{
			...
			var result = ServiceLayer.InsertUpdateData("InsertUpdateRole",data);
	...
	deleteRole:function()
	{
	   ...
	   {	
			...
			var result = ServiceLayer.DeleteData("DeleteRole",data); 
			if(result != '' && result != undefined)
			{
				if(result[0].Value == "Done")		
				{
   		...
   		if($.trim($('#txtNewRole').val())!='')
		{
		  ...
		  var iChars = "!@#$%^&*()+=[]\';,/{}|\":<>?";
			...
			if(flagk == 0)  
			{
				...
				if(result != '' && result != undefined)
				{
					if(result[1].Value == "Done")
					{
						var item = {}
				        item ["roleId"] = result[0].Value;
	...
	configRole:function()
	{
		...
		$('#confRole').dialog({ resizable: false,
						modal: true,
						height: 370,
						width:500,
						buttons: {
	...
	{
		if($("#testerRoles div div li").find(":checked").length == "0")
			tester.alertBox("Please select the "+tester.gConfigRoles.toLowerCase()+" first!");//Added by Mohini for Resource file
		else if($("div[Title='tester']").text()=='' || $("div[Title='tester']").text()== null || $("div[Title='tester']").text() == undefined)
			tester.alertBox("Enter the "+tester.gConfigTester+"(s) first!");//Added by Mohini for Resource file
			else
			{
			    ...
			    var principalType = Main.SearchPrincipalType( displayText )
				...
				else
				{
					...
					AddUser.globTesterName = tester.resolveTesterName(TesterStr);
					...
					newSPUserIdN = AddUser.testerSPID;
				  ...
				  //tester old role from list in ttoldroles
					...
					if(!blockDelete)
					{
						...
						var TesterStr=$("textarea[Title='tester']").val();
						var testerDisplayName=tester.resolveTesterFullName(TesterStr);
		            	var testerEmail=tester.resolveTesterEmail(TesterStr);
						...
						var result = ServiceLayer.InsertUpdateData("InsertUpdateTester",data);
	...
	deleteTester:function(spUserId)
	{
		...
		{
			var TestCaseResult = ServiceLayer.GetData("GetTestCaseDetailForTestPassId",show.testPassId);
				...
				({
			         ...
			         buttons: 
			         {
						"Delete": function() {		
												...
												var result = ServiceLayer.DeleteData("DeleteTester",obj);
												if(result != undefined && result != '')
												{
													if(result[0].Value == "Done")
													{
	...
	validationOnSave:function()//function get call on save
    {
	    ...
	    //#Rupal
			...
			if(tester.gPrincipalType=='User')
			{
				...
				newSPUserIdN = tester.resolveSPUserID($("textarea[Title='tester']").val());
					 ...
					 else
					 {
						...
						var data={
				        		'testPassId':show.testPassId,
  								...
  								'testerName':testerDisplayName[ss][0],
  								'testerAlias':testerName[ss][0],
  								'testerEmail':testerEmail[ss][0],
  								'areaName':area,
  								'areaId':areaId,
  								'roleArray':roleIdArray,
  								'action':'add'

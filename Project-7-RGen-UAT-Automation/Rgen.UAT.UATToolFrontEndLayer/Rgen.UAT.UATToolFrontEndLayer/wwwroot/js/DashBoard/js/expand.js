       ...
       (o.switchPosition == 'before') ? ($.fn.findSibling = $.fn.prev, $.fn.insrt = $.fn.before) : ($.fn.findSibling = $.fn.next, $.fn.insrt = $.fn.after);
        ...
        $sw.click(function() {
            ...
            if ($switch.text() == o.expTxt) {
              if (o.oneSwitch) {teststepfortester.OnShow_fillGrid();$switch.text(o.cllpsTxt).attr('class', 'open');/*alert('show');$(".demo4").show("fast");*/}
              $tr.addClass('open').data('state', 1);
              $cllps[o.showMethod](o.speed);
            } else {
              if (o.oneSwitch) {$switch.text(o.expTxt).attr('class', '');}
              $tr.removeClass('open').data('state', 0);
              if (o.speed == 0 || o.instantHide) {$cllps.hide();} else {teststepfortester.OnHide_fillGrid();$cllps[o.hideMethod](o.speed);/*alert('hide');*/}
...
$.fn.expandAll.defaults = {
        ...
        expTxt : '[Show Test Pass Details]', // the text of the switch for expanding
        cllpsTxt : '[Hide Test Pass Details]', // the text of the switch for collapsing

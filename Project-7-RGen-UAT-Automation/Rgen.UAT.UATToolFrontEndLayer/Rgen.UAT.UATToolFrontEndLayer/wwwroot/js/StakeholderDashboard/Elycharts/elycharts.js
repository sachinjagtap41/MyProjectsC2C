  ...
  common : {
    ...
    */
    ...
    features : {
      ...
      legend : {
        horizontal : true,
        x : 50, // X | auto, (auto solo per horizontal = true)
        y : 380, 
        ...
        borderProps : { fill : "white", stroke : "white", "stroke-width" : 1 },
        dotType : 'rect',
        dotWidth : 10, dotHeight : 10, dotR : 4,
...
commonPie : {
    ...
    defaultSeries : {
      ...
      tooltip : {
        ...
        offset: [20, 0] /* y, x */,
        frameProps : { fill: "white", "stroke-width": 2 },
        contentStyle : { "word-wrap":"break-word","font-family": "Arial", "font-size": "12px", "line-height": "16px", color: "black" }
      },
    ...
    features : {
      ...
      legend : {
        horizontal : true,
        x : 400, // X | auto, (auto solo per horizontal = true)
        y : 380, 
        width : 'auto', // X | auto, (auto solo per horizontal = true)
        height : 400,
        itemWidth : "fixed", // fixed | auto, solo per horizontal = true
  ...
  line : {
    ...
    features : {
      grid : {
        ...
        forceBorder : false,
        // Proprieta' di visualizzazione griglia // props : {stroke: '#e0e0e0', "stroke-width": 1},
        props : {stroke: '#C0C0C0', "stroke-width": 1},
...
$.elycharts.tooltipmanager = {
  ...
  getTip : function(env, serie, index) {
    ...
    //HC
   /* var project = report.arrProjects[index];

    $("#projectChart").unbind('click');
	$("#projectChart").mouseover(function(){ $("#projectChart").css("cursor","pointer"); }).click(function(z){
		$("#tabs").css("display","none");
		var status = report.arrSeries[serie.substr(serie.length - 1)];
		var project = report.arrProjects[index];
		$("#hProjectName").val(project);
		report.showTestpassForStatus(project, status);	
	$("#projectChart").unbind('click');
		}
	);
	
	
	$("#testpassChart").unbind('click');
	$("#testpassChart").mouseover(function(){ $("#testpassChart").css("cursor","pointer"); }).click(function(z){
		$("#tabs").css("display","none");
		var status = report.arrSeries[serie.substr(serie.length - 1)];
		var testpass = report.arrTestPass[index];
		report.showTesterAndTCForStatus(testpass , status);	
	$("#testpassChart").unbind('click');
		}
	);*/

	$("#pieChartDet").unbind('click');
	$("#pieChartDet").mouseover(function(){ $("#pieChartDet").css("cursor","pointer"); }).click(function(z){
		$("#dvtabs").css("display","none");
		//debugger;
		//alert(report.arrSeries[index]);
		$("#dvTester").css("display","none");
		report2.showDataForProjectStatus(report.arrSeriesPie[index]);
		//alert();
	$("#pieChartDet").unbind('click');
		}
	);
	
	$("#testerChart").unbind('click');
	$("#testerChart").mouseover(function(){ $("#testerChart").css("cursor","pointer"); }).click(function(z){
		$("#tabs").css("display","none");
		
		var status = report.arrSeries[serie.substr(serie.length - 1)];
		var tester = report.arrTesterName[index];
		var SPID = report.forTesterNameSPID[tester];
		report.showTesterGrid(SPID,tester , status);	
		report.createFilterContainerPostFilter();
	$("#testerChart").unbind('click');
		}
	);

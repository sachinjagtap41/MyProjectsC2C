...
var multiSelectList = {
    ...
    createMultiSelectList: function (divID, listItems, columnNameForText, columnNameForValue, height, signUp) {
        /*Code added by Deepak for implementation of sequencing 16-4-13*/
        var divhtml = "";
        if (divID != "testerRoles" && divID != "testerRolesConf") {

            if (divID == "pjctID" || divID == "tpID" || divID == "testMgr") {
                divhtml += "<div class='Mediumddl' style='border: solid 1px #ccc;  width:inherit; padding-left:1px;'>"
                ...
                + "<ul id='ulItems" + divID + "' style='list-style-type:none; list-style-position:outside;display:inline;'>" +
                    ...
                    "<div style='overflow-y:scroll; height:" + height + " width:300px'>";
            }
        }
        else {
        ...
        for (var i = 0; i < listItems.length; i++) {
            ...
            if (listItems[i][columnNameForText] != undefined) {
                ...
                if (divID == 'testStepName') {
                    ...
                    if (divID == "testerRoles" || divID == "testerRolesConf") {
                        if (listItems[i][columnNameForText] != "Standard") {
                            if (listItems[i]['roleDetails'] != undefined)
                                var desc = listItems[i]['roleDetails'];
                            else
                                var desc = "";
                            divhtml = divhtml + "<li title='" + multiSelectList.filterData(hoverTxt).trim() + "'><input id='" + itemId + "'  type='checkbox' class='mslChk'></input><span id='" + itemId + "' style=\"display: none;\" title='" + desc + "'>" + multiSelectList.filterData(listItems[i][columnNameForText]) + "</span>" + trimText(multiSelectList.filterData(listItems[i][columnNameForText]), 100) + "</li>";
                        }
                    }
                    else {
    ...
    createRoleMultiSelectList: function (divID, listItems, columnNameForText, columnNameForValue, height) {
        var divhtml = "<div class='Mediumddl' style='border: solid 1px #ccc;  width:390px; padding-left:1px;'>" +

                    "<ul id='ulItems" + divID + "' style='list-style-type:none; list-style-position:outside;display:inline;'>" +
                        "<li>Select:&nbsp;<a id='linkSA_" + divID + "' style='color:blue; text-decoration:underline; cursor:pointer' onclick='multiSelectList.selectAll(\"" + divID + "\");'>All</a>" +
                             "&nbsp;&nbsp;&nbsp;<a id='linkSN_" + divID + "' style='color:blue; text-decoration:underline; cursor:pointer' onclick='multiSelectList.selectNone(\"" + divID + "\");'>None</a>" +
                             "&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;Show:&nbsp;<a id='anchShow_" + divID + "' style='color:blue; text-decoration:underline; cursor:pointer' onclick='multiSelectList.toggleSelectionDisplay(\"" + divID + "\");'>Selected</a></li>" +
                        "<li><hr/></li>" +
                        //"<div style='overflow-y:scroll; height:"+multiSelectList.height+" width:290px'>";
                        "<div style='overflow-y:scroll; height:" + height + " width:390px'>";
        ...
        if (FromExport == 'true') {
            ...
            if (length > 0) {
                ...
                for (i = 0; i <= length; i++) {
                    ...
                    /*for(k=0;k<=pElement.childNodes.length-1; k++)
                    {
                        //
                        var pFirstSpan= pElement.childNodes[k];
                        
                        if(pFirstSpan.childNodes.length>0 && pFirstSpan.nodeName=='SPAN')
                        {
                            var pSecondSpan= pFirstSpan.childNodes[0];
                            
                            if(pSecondSpan.childNodes.length>0 && pSecondSpan.nodeName=='SPAN')
                            {
                    ...
                    if (pElement.childNodes.length >= 2) {
                        ...
                        //var txtText =$('#dvTemp').find('p')[i].innerText;
                    }
                    else {

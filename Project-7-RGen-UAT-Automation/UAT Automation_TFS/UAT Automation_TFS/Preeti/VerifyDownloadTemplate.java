...
public class VerifyDownloadTemplate extends TestSuiteBase{
					...
					//create test pass
	            	 if(!createTestPass(groupName, portfolioName, projectName, version, testPassName, testPassEndMonth, testPassEndYear, testPassEndDate, testManager.get(0).username))
	            	 {
	            		     fail=true;
		                   	 ...
		                   	 comments+="Fail - Test Pass not Created Successfully. ";
		                   	 assertTrue(false);
	...
	private boolean verifyGuidelineMsgAndDownloadTemplateFunction(String project, String version, String testPass, String testerRole, String testStepResult, 
			String importTestStepGuidelineMessage)
	{
		...
		try
		{
			...
			for (int i = 0; i < totalPages; i++) 
			{
				...
				for (int j = 1; j <= myActivityTableRows; j++) 
				{
					...
					{
							//Ekta, its not verifying that it should be. Rather its just checking if it is.
							//No problem if its not. There should have been assertion here
							//verifying Days Left & Not Completed count
		                    if((daysLeft>0) && (notCompletedCount>0 || notCompletedCount!=0) && (passCount==0) && 
		                       (failCount==0) && (actionCell.equals("Begin Testing")) &&(offlineTestingCell.equals("Download Testing Template")))
							{
		                		...
		                		Thread.sleep(4000);
		                		APP_LOGS.debug(testPass+" Found in My Activity Table and Template downloaded successfully.");
		                		return true;
							}
					}
				}

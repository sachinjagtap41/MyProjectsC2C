			 ...
			 if(isLoginSuccess)
			 {
					    try
					    {
							...
							else
							{
								...
								if(assertTrue(isElementExistsByXpath("DashboardProjectTestPassSummary_NoTestPassLabel")))
								{
								...
								{
									fail=true;
									APP_LOGS.debug("'No Test Pass Available' label not found(Fail). ");
							    	comments+="'No Test Pass Available' label not found(Fail). ";
								}

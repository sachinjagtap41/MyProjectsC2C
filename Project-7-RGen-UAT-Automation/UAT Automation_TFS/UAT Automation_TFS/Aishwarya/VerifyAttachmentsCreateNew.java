...
public class VerifyAttachmentsCreateNew extends TestSuiteBase {
		...
		if(isLoginSuccess)
		{
			try{
                ...
                APP_LOGS.debug("Opening Browser... ");
 				...
 				//VALIDATION
 					boolean blankNameMessageResult = validation(expectedAttachmentNameBlankMessage);
				...
				//Checking validation after entering attachment name
					...
					boolean blankTestStepMessageResult = validation(expectedTestStepBlankMessage);
				  ...
				  //Checking validation  after entering attachment name and test step
						...
						boolean blankFileFieldMessageResult = validation(expectedFileNameBlankMessage);
			...
			catch(Throwable t)
            {
         	 fail=true;
           	 t.printStackTrace();
           	 comments="Fail- Unable to click on Test Management Tab";
	...
	public boolean validation(String expectedfieldBlankMessage) throws IOException, InterruptedException{
		...
		public int testCaseDropdown (WebElement testCaseDD, String testCaseName)
		{
		 ...
		 public int selectTestStep(String TestStep)
		 {
		  List<WebElement> testStepCount = getObject("AttachmentsCreateNew_testStepDetailsBox").findElements(By.tagName("li"));
		     System.out.println("Step Count: " +testStepCount.size());
			    ...
			    try
			    {
				    ...
				    String actual_SaveAttachment_SuccessMessage = eventfiringdriver.findElement(By.xpath("//div[@id='divAlert']")).getText();
					 ...
					 //  Assert.assertEquals(actual_SaveAttachment_SuccessMessage, AttachmentSavedMessage);
					   compareStrings(actual_SaveAttachment_SuccessMessage, AttachmentSavedMessage);

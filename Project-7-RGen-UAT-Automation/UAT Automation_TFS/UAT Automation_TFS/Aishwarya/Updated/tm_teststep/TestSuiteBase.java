...
public class TestSuiteBase extends TestBase{
	...
	// check if the suite ex has to be skiped
		...
		public void checkSuiteSkip() throws Exception{
			initialize();
			APP_LOGS.debug("Checking Runmode of TM_TestSteps Suite");
			if(!TestUtil.isSuiteRunnable(suiteXls, "TM_TestSteps Suite")){
				APP_LOGS.debug("Skipped TM_TestSteps Suite  as the runmode was set to NO");
				throw new SkipException("Runmode of TM_TestSteps Suite set to NO ... So Skipping all tests in TM_TestSteps Suite");
			}
		...
		public boolean createProject(String group, String portfolio, String project, String version, String endMonth, String endYear, String endDate, String versionLead ) throws IOException, InterruptedException
		{
			...
			try {
				...
				//	String projectCreatedResult=wait.until(ExpectedConditions.visibilityOf(getElement("ProjectCreateNew_projectSuccessMessageText_Id"))).getText();
				...
				String endMonth, String endYear, String endDate, String testManager ) throws IOException, InterruptedException
		{
			...
			try 
			{
				...
				//	String testPassCreatedResult=wait.until(ExpectedConditions.visibilityOf(getElement("TestPassCreateNew_testPassSuccessMessageText_Id"))).getText();
				...
				String tester, String testerRoleCreation, String testerRoleSelection) throws IOException, InterruptedException
		{
			...
			//		String testerCreatedResult=wait.until(ExpectedConditions.visibilityOf(getElement("TesterCreateNew_testerSuccessMessageText_Id"))).getText();
				...
				String testStepName, String testStepExpectedResults, String testCasesToBeSelected, String rolesToBeSelected) throws IOException, InterruptedException
		{
			...
			try 
			{
					...
					//	String testStepCreatedResult=getElement("TestStepCreateNew_testStepSuccessMessageText_Id").getText();
		...
		public void dropDownSelect(WebElement dropDownList,WebElement SelectionList, String selectionText) throws IOException, InterruptedException 
		{
			...
			{
					if(elements.get(i).getAttribute("title").equals(selectionText))
					{
		...
		protected boolean searchForTheTestStep(String testStepName) throws IOException, InterruptedException
		{
			int totalPages;
			int testStepOnPage;
			String gridTestStep;
			...
			try
			{
				if (getElement("TestStepViewAll_Pagination_Id").findElements(By.xpath("div/span")).size()==3) 
				{
				...
				for (int i = 0; i < totalPages; i++) 
				{
					...
					for (int j = 1; j <= testStepOnPage; j++) 
					{
						...
						{
							getObject("TestStepViewAll_editProjectIcon1", "TestStepViewAll_TestStepEditIconXpath2", j).click();
							Thread.sleep(1000);
							APP_LOGS.debug("Test Step Found in View All page.");
							return true;
						}
					}			
					if (totalPages>1 && i!=(totalPages-1)) 
					{
						getObject("TestStepViewAll_NextLink").click();
					}
				}
				//assertTrue(false);
				APP_LOGS.debug("Test Step Not found in View All page");
		...
		protected boolean verifyTestStepFields(String testStepName, String expectedResult, String testCaseName, String roles) throws IOException, InterruptedException
		{
			String displayedtestStepName;
			String displayedexpectedResult;
			String displayedtestCaseName = null;
			String displayedroles= null;
			
			try
			{	
				JavascriptExecutor js = (JavascriptExecutor) eventfiringdriver;
				
				displayedtestStepName= (String) js.executeScript("return $(document).contents().find('#rte1').contents().find('body').text().trim()");
				
				displayedexpectedResult=getElement("TestStepCreateNew_testStepExpectedResults_ID").getText();
				
				List<WebElement> TestCaseSelectionNames = getObject("TestStepCreateNew_SelectionTestCaseList").findElements(By.tagName("li"));
				int numOfTestCases = TestCaseSelectionNames.size();
				for(int i = 0; i<numOfTestCases;i++)
				{
						if((getObject("TestStepCreateNew_SelectionTestCaseList_Xpath1", "TestStepCreateNew_SelectionTestCaseList_Xpath2", (i+1)).isSelected()))
						{
							displayedtestCaseName=TestCaseSelectionNames.get(i).getText()+",";
						}
				}
				
				displayedtestCaseName= displayedtestCaseName.substring(0, displayedtestCaseName.length()-1);
				
				List<WebElement> roleSelectionNames = getObject("TestStepCreateNew_SelectionRoleList").findElements(By.tagName("li"));
				int numOfRoles = roleSelectionNames.size();
				for(int i = 0; i<numOfRoles;i++)
				{
					if(getObject("TestStepCreateNew_SelectionRoleList_Xpath1", "TestStepCreateNew_SelectionRoleList_Xpath2", (i+1)).isSelected())
					{
						displayedroles = roleSelectionNames.get(i).getAttribute("title")+",";
					}
				}
				
				displayedroles = displayedroles.substring(0, displayedroles.length()-1);			
				
				if (assertTrue(displayedtestStepName.equals(testStepName) && displayedexpectedResult.equals(expectedResult) && 
						displayedtestCaseName.equals(testCaseName) && displayedroles.equals(roles))) 
				{
					return true;
				}

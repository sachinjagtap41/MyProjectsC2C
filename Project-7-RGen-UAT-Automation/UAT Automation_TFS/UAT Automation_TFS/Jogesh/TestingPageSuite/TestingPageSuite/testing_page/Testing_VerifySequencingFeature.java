...
// Ekta Static variables were not required
	static int count=-1;
	...
	static String tp2selectedTestCaseName2;
	static	int countForPassedTestSteps=0;
	static int countForFailedTestSteps=0;
	static	int countForNotCompletedTestSteps=4;
	static	int countForPendingTestSteps=0;
	static	String selectTestCaseFromDropdown;
	String comments;
	...
	// Test Case Implementation ...
									...
									String feedback,String properSequencingPopUpMessage) throws Exception
		{
			...
			if(isLoginSuccess)
			{
					...
					//Aishwarya----	APP LOGS should be present after creating the data as Creating statement is already present in create functions. 				
			   ...
			   APP_LOGS.debug("Creating Test Pass :" + testPassName4+ " with Test Cases : " + tp4TestCase1 + " with their respective Test Steps  ." );
				...
				try
				{
					...
					System.out.println("The size of Select Test Pass drop down" + selectTP.size());
				...
				APP_LOGS.debug("Opening Browser...Logging In With Tester "+tester.get(0).username + " for ");
				...
				if (login(tester.get(0).username,tester.get(0).password, "Tester")) 
				{
					...
					if(actualTestPassName2PresentOnTestingPage.equals(testPassName2))
					{
						...
						List<WebElement> selectTC = getElement("TestingPageMyActivityTestCasesDropdown_Id").findElements(By.tagName("option"));
						System.out.println("The size of Select Test Case drop down" + selectTC.size());
...
public Object[][] getTestData(){
	...
	private boolean searchTestPassAndPerformTesting(String project, String version, String testPass, String testStepName1,String testStepName2,String testerRole, String action)
	{
		...
		try
		{
			...
			for (int i = 0; i < totalPages; i++) 
			{
				...
				for (int j = 1; j <= myActivityTableRows; j++) 
				{
					...
					if ((projectCell.equals(project)) && (versionCell.equals(version)) && (testPassCell.equals(testPass)) && (roleCell.equals(testerRole)))
					{
		                    ...
		                    //Ekta return true is placed in wrong location
		                    return true;
				 ...
				 if (totalPages>1) 
			      {
			         eventfiringdriver.findElement(By.xpath("//div[@id='MyActPagination']/div/a[text()='Next']")).click();
			...
			System.out.println("project Not FOund in My Activity");
			return false;	
			
	}
//Ekta catch should have assertTrue(false)
		catch(Throwable t)
		{
			fail=true;
			...
			String testStepName, String testStepExpectedResults, String testCasesToBeSelected, String rolesToBeSelected) throws IOException, InterruptedException
	{
		...
		
		{
				...
				dropDownSelect(getElement("TestStepUpperRibbon_testPassDropDown_Id"), getElement("TestStepUpperRibbon_TestPassList_Id"), testPassName );
					...
					{
						Thread.sleep(1000);
						System.out.println(testStepName+t);

...
public class VerifyDescriptionTableArea extends TestSuiteBase{
						...
						if(login(tester.get(0).username,tester.get(0).password, "Tester"))
						{
				 	       ...
				 	       if (login(tester.get(1).username,tester.get(1).password, "Tester")) 
							{
								...
								if (login(testManager.get(0).username,testManager.get(0).password, "Test Manager")) 
								{
									 ...
									 List<WebElement> listOfProjects=getElement("DetailedAnalysis_projectDropDown_Id").findElements(By.tagName("option"));
									 selectDDContent(listOfProjects, ProjectName, "Project");
									 
									 List<WebElement> listOfVersion=getElement("DetailedAnalysis_versionDropDown_Id").findElements(By.tagName("option"));
									 selectDDContent(listOfVersion, Version, "Version");
									 
									 
									 List<WebElement> listOfTestPass=getElement("DetailedAnalysis_testPassDropDown_Id").findElements(By.tagName("option"));
	...
	private void selectDDContent(List<WebElement> listOfElement, String selectElement, String element)
	{
		for(int i = 0;i<listOfElement.size();i++)
	     {	
			if(listOfElement.get(i).getAttribute("title").equalsIgnoreCase(selectElement))
			{

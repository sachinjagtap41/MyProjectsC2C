/**
 * @author preeti.walde
 * Created: 27th Jan 2015
 * Last Modified: 27th Jan 2015
 * Description: Code to test 'FY15, FY14' options in Fiscal Year dropdown in Consolidated View tab.
 */
...
public class VerifyCVFiscalYearDropdown extends TestSuiteBase
{
	...
	public void verifyCVFiscalYearDropdown(String Role, String FiscalYear15, String FiscalYear14, String Quarter) throws Exception
	{
		...
		{	
				try
					{		
					    ...
					    int fiscalYear15 = Integer.parseInt(FiscalYear15.substring(3));					
					    int fiscalYear14 = Integer.parseInt(FiscalYear14.substring(3));
						...
						if(numOfProjectPresentInGrid==0)
						{
							...
							if(noTestStepAvailableTagSize==1)
							{
								...
								{
									APP_LOGS.debug("No Projects Available for "+fiscalYear15+" and "+quarter+" filter criteria..verified 'No Test Steps Available!' message");
									comments+="No Projects Available for "+fiscalYear15+" and "+quarter+" filter criteria..verified 'No Test Steps Available!' message.(Pass) ";
								}
								else
								{
									fail=true;
									APP_LOGS.debug("'No Test Step Available' message not shown though no projects available for selected "+fiscalYear15+" and "+quarter+" filter criteria");
									comments+="'No Test Step Available' message not shown though no projects available for selected "+fiscalYear15+" and "+quarter+" filter criteria.(Fail) ";
									TestUtil.takeScreenShot(this.getClass().getSimpleName(), "'No Test Steps Available' message not shown");
								}
							}
							
							
							if(compareIntegers(FY15OptionArray.length, numOfProjectPresentInGrid))
							{
								APP_LOGS.debug("Displayed project count "+numOfProjectPresentInGrid+" is same as FY15OptionArray count "+FY15OptionArray.length+".");
							}
							else
							{
								fail=true;
								comments += "\n Displayed project count "+numOfProjectPresentInGrid+" is same as FY15OptionArray count "+FY15OptionArray.length+".(Fail) ";						
								APP_LOGS.debug("Displayed project count "+numOfProjectPresentInGrid+" is same as FY15OptionArray count "+FY15OptionArray.length+". Test case failed");
							}
						}
						else
						{
								//verify projects for All option
								verifyProjectSection(FY15OptionArray);
						}
												
						//verify project status, select date, active tester, execution bar graph, executed status count options
						verifyDropdownsAndActiveTesterFields();
						
						
						
						//select FY 14 option from fiscal year dropdown						
						selectFiscalYearDD.selectByVisibleText(FiscalYear14);
			
						//select Q4 checkbox to verify projects for FY14 and Q4
						if(!getElement("StakeholderDashboard_Q4Checkbox_Id").isSelected())
						{
							getElement("StakeholderDashboard_Q4Checkbox_Id").click();
						}
						...
						//this array will collect all the projects which falls under selected quarter and fiscal year						    
					    String[][] FY14OptionArray = getProjectsForReqFYAndQuarter(fiscalYear14,quarter,projectDetails);
						...
						if(numOfProjectPresentInGrid==0)
						{
								...
								if(noTestStepAvailableTagSize==1)
								{
									if(compareStrings(resourceFileConversion.getProperty("StakeholderDashboardCV_noTestStepAvailableMsg"), getElement("StakeholderDashboardConsolidatedView_statusPieChart_Id").getText()))
									{
										APP_LOGS.debug("No Projects Available for "+fiscalYear14+" and "+quarter+" filter criteria..verified 'No Test Steps Available!' message");
										comments+="No Projects Available for "+fiscalYear14+" and "+quarter+" filter criteria..verified 'No Test Steps Available!' message.(Pass) ";
									}
									else
									{
										fail=true;
										APP_LOGS.debug("'No Test Step Available' message not shown though no projects available for selected "+fiscalYear14+" and "+quarter+" filter criteria");
										comments+="'No Test Step Available' message not shown though no projects available for selected "+fiscalYear14+" and "+quarter+" filter criteria.(Fail) ";
										TestUtil.takeScreenShot(this.getClass().getSimpleName(), "'No Test Steps Available' message not shown");
									}
									
								}
								
								if(compareIntegers(FY14OptionArray.length, numOfProjectPresentInGrid))
								{
									APP_LOGS.debug("Displayed project count "+numOfProjectPresentInGrid+" is same as FY14OptionArray count "+FY14OptionArray.length+".");
								}
								else
								{
									fail=true;
									comments += "\n Displayed project count "+numOfProjectPresentInGrid+" is same as FY14OptionArray count "+FY14OptionArray.length+".(Fail) ";						
									APP_LOGS.debug("Displayed project count "+numOfProjectPresentInGrid+" is same as FY14OptionArray count "+FY14OptionArray.length+". Test case failed");
								}
						}
						else
						{
								//verify projects for All option
								verifyProjectSection(FY14OptionArray);
						}
						...
						projectDetails=null;
						projectArrayList.clear();
						FY15OptionArray=null;
						FY14OptionArray=null;
	...
	private String[][] getProjectsForReqFYAndQuarter(int fiscalYear ,int quarter, String[][] projectArrayToSearchIn)
	{
		...
		{	
			...
			projectStartDate = projectArrayToSearchIn[i][6];
			...
			if ((((getFiscalYear(projectStartDate)==fiscalYear && getQuarter(projectStartDate) == quarter))
					...
					&& (projectArrayToSearchIn[i][7]!=null)) 
			{
		...
		for (int j = 0; j < projectDetailsForReqFYAndQuarter.length; j++) 
		{
			...
			projectDetailsForReqFYAndQuarter[j][7] = projectArrayToSearchIn[projectRowToGetDetailsFrom][7];
		}
	...
	private void verifyDropdownsAndActiveTesterFields()
	{
		try
		{
			...
			//verify if active tester countis visible or not			
	...
	private void verifyProjectSection(String[][] filterArray) throws IOException
	{
		...
		{							
				...
				{
					APP_LOGS.debug("Displayed project count "+projectVerifiedCount+" is same as "+filterArray+" count "+filterArray.length+".");
				}
				else
				{
					fail=true;
					comments += "\n Displayed project count "+projectVerifiedCount+" is same as "+filterArray+" count "+filterArray.length+".(Fail) ";						
					APP_LOGS.debug("Displayed project count "+projectVerifiedCount+" is same as "+filterArray+" count "+filterArray.length+". Test case failed");
				}

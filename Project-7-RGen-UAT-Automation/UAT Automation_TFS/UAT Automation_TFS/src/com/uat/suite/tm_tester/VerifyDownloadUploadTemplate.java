		...
		if(isLoginSuccess)
		{
			...
			//	wait.until(ExpectedConditions.elementToBeClickable(By.id("downloadT")));
				...
				//	enterTesterName(tester.get(0).username);
					String[] tname = retrieveNameOfTester(System.getProperty("user.dir")+"\\Download_UploadTemplate\\Already Exist tester Upload.xlsx");
									
					for(int numOfTester=0;numOfTester<tname.length;numOfTester++)
					{
						System.out.println(tname[numOfTester]);
						enterTesterName(tname[numOfTester]);
				...
				APP_LOGS.debug("Executing UPLOAD ALREADY EXIST TESTERS test case");

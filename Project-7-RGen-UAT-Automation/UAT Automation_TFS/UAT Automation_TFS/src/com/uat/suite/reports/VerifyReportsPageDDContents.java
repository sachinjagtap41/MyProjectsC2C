 ...
 * Last Modified on Date:-10th Feb 2015
...
public class VerifyReportsPageDDContents extends TestSuiteBase{
		...
		if(isLoginSuccess)
		{
			 ...
			 try
			 {
				 if(login(test_Manager.get(0).username,test_Manager.get(0).password, "Test Manager"))
				 {
					...
					testerDropDown = new Select(getElement("Triage_testerDropDown_Id"));
					...
					{
						optionInTestersDD.add(testerDropDown.getOptions().get(i).getText());
					...
					if (iflag != 1) 
					{
						 APP_LOGS.debug("Testers are present in Testers dropdown(Pass).");				                	 
	               	 
						 comments+="Testers are present in Testers dropdown(Pass).";				                	 
					}
					else 
					{
						 fail=true;					
						 ...
						 APP_LOGS.debug("Testers are not present in Testers dropdown(Fail).");				                	 
	               	 
						 comments+="Fail Occurred:- Testers are not present in Testers dropdown(Fail).";				                	 
						
						 TestUtil.takeScreenShot(this.getClass().getSimpleName(), "TestersNotFoundInDD.");
					}  
					
					//Selecting Tester
					testerDropDown.selectByVisibleText(testersArray.get(1));
	...
	{	
		...
		//Veridy if Export button is visible or not

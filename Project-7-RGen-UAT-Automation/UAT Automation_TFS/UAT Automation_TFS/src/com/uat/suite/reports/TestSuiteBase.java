...
public class TestSuiteBase extends TestBase
{
	...
	// check if Reports Suite has to be skipped
	@BeforeSuite
	public void checkSuiteSkip() throws Exception
	{
		initialize();
		APP_LOGS.debug("Checking Runmode of Reports Suite");
		if(!TestUtil.isSuiteRunnable(suiteXls, "Reports Suite"))
		{
			APP_LOGS.debug("Skipped Reports Suite  as the runmode was set to NO");
			throw new SkipException("Runmode of Reports Suite set to NO ... So Skipping all tests in Reports Suite");
		}
	}
	...
	//		String testCaseCreatedResult=(String) eventfiringdriver.executeScript("return $('#autoHideAlert').text();");
			...
			String testStepName, String testStepExpectedResults, String testCasesToBeSelected, String rolesToBeSelected) throws IOException, InterruptedException
	{
		...
		//	getElement("TestStepCreateNew_testStepSaveBtn_Id").sendKeys(Keys.ENTER);
		...
		//	String testStepCreatedResult=(String) eventfiringdriver.executeScript("return $('#autoHideAlert').text();");
			...
			String testStepCreatedResult = getTextFromAutoHidePopUp();
		...
		*/	
		...
		//	while(nextLinkEnabled==true || paginationCount==1)
			...
			/*	if (totalPages>1 && projectFound==false)
				...
				nextLinkEnabled = isElementExistsByXpath("DashboardProjectTestPassSummary_nextLink");  */
			}while(ifElementIsClickableThenClick("DashboardMyActivity_NextLink"));
			APP_LOGS.debug("Project, version and test pass Not found in View All page. Test case has been failed.");
			return false;
		}
		catch(Throwable t)

 ...
 * Created Date:- 4th Feb 2015
 * Last Modified on Date:- 4th Feb 2015
 */
...
public class VerifySequentialTestingWithinTC extends TestSuiteBase
{
						...
						if ( getElement("Configuration_testingTypeOptionRadionButton_Id").isSelected() ) 
						{
							...
							if (login(tester.get(0).username,tester.get(0).password, "Tester")) 
							{
								...
								if (compareStrings(resourceFileConversion.getProperty("Testing_CompleteTestingInProperSequenceAlertText"),actualAlertText )) 
								{
									...
									APP_LOGS.debug("'You must complete Test Steps in proper Sequence' alert message is displayed while sequence of Test step didn't follow.");
									
									comments += "\n- 'You must complete Test Steps in proper Sequence' alert message is displayed while sequence of Test step didn't follow.";
								...
								else 
								{
									...
									APP_LOGS.debug("'You must complete Test Steps in proper Sequence' alert message is NOT displayed while sequence of Test step didn't follow.(Fail) ");
									
									comments += "\n- 'You must complete Test Steps in proper Sequence' alert message is NOT displayed while sequence of Test step didn't follow. (Fail)";
								...
								String pointerImageTS1 = getObject("TestingPage_testStepSequence", "TestingPage_testStepLink2", 1).getAttribute("style");
								
								String pointerImageTS2 = getObject("TestingPage_testStepSequence", "TestingPage_testStepLink2", 2).getAttribute("style");
								
								if (pointerImageTS2.contains("background-image") && (! pointerImageTS1.contains("background-image"))) 
								{
									APP_LOGS.debug("Test Step pointer has been pointed to next Test Step in sequence.");
									
									comments += "\n- Test Step pointer has been pointed to next Test Step in sequence.";
								...
								else 
								{
									...
									APP_LOGS.debug("Test Step pointer has NOT been pointed to next Test Step in sequence.(Fail) ");
									
									comments += "\n- Test Step pointer has NOT been pointed to next Test Step in sequence. (Fail)";
									
									TestUtil.takeScreenShot(this.getClass().getSimpleName(), "Test Step pointer has NOT been pointed to test step 2");

...
public class TestSuiteBase extends TestBase{
	...
	public void checkSuiteSkip() throws Exception{
		initialize();
		APP_LOGS.debug("Checking Runmode of Configuration Suite");
		if(!TestUtil.isSuiteRunnable(suiteXls, "Configuration Suite"))
		{
			APP_LOGS.debug("Skipped Configuration Suite  as the runmode was set to NO");
			throw new SkipException("Runmode of Query Suite set to NO ... So Skipping all tests in Configuration Suite");
		}
	}
	...
	public boolean isNextLinkEnable(String nextLink) throws IOException
	 {
	 ...
	 public boolean isPreviousLinkEnable(String previousLink) throws IOException
	 {
		  boolean isPreviousLinkEnabled = true;
		    
		  while( isPreviousLinkEnabled )  
		  {
		    try
		    {
		     
			     if( isLinkDisabled(previousLink) == null)
			     {
				      setImplicitWait(0);      

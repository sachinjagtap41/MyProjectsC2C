...
public class Testing_FeedbackRatingOnEachTC extends TestSuiteBase
{
   ...
   //Test Case Implementation ...	
					    ...
					    */
	    				...
	    				{   
	    					...
	    					if(!searchTestPassAndPerformTesting(ProjectName, Version, testPass[0],TestStepDetails1,TestStepDetails2, TesterRole1, TestStepResult1, Rating, Feedback, FeedbackRatingPopupText, "Begin Testing", feedbackNote,"Go to Feedback"))
	    					{
	    						fail = true;
	    						assertTrue(false);
	    						//throw new SkipException("Testing for tester " +tester.get(0).username+" of " +testPass[0]+ " with role " +TesterRole1+ " is unsuccessful.");
	    					}
	        				...
	        				if(!searchTestPassAndPerformTesting(ProjectName, Version, testPass[1],TestStepDetails1,TestStepDetails2, TesterRole2, TestStepResult2, Rating, Feedback, FeedbackRatingPopupText, "Begin Testing", feedbackNote, "Return to Home"))
	    					{
	        					fail = true;
	    						assertTrue(false);
	    						//throw new SkipException("Testing for tester " +tester.get(0).username+" of " +testPass[1]+ " with role " +TesterRole2+ " is unsuccessful.");
	    					}
	        				...
	        				if(!searchTestPassAndPerformTesting(ProjectName, Version, testPass[0],TestStepDetails1,TestStepDetails2, TesterRole1, TestStepResult1, Rating, Feedback, FeedbackRatingPopupText, "Testing Complete", feedbackNote,""))
	        				{
	        					fail = true;
	    						assertTrue(false);
	        					//throw new SkipException("Testing for tester " +tester.get(0).username+" of " +testPass[1]+ " with role " +TesterRole1+ " is unsuccessful.");
	        				}
	    				}
	    				else
	    				{
	...
	private void configuationSettingFeedbackRating(String ProjectName ,String Version, String TestPassName) {
							...
							{	
	                    		...
	                    		for (int a = testStepResultToBe; a < testSteps1.length; a++) 
	                    		{
	                    		 ...
	                    		 if((passCount!=0)&&(failCount==0))
	                    		 {
		                    	   ...
		                    	   {	
                    				   //Calling function for providing the feedback
                    				   if(!testPassFeedbackRatingPopUp(expectedFeedbackRatingPopUpTitle, Rating, Feedback))
                    				   {
                    					   return false;
                    				   }
	                    		...
	                    		{	                    			
	                    			APP_LOGS.debug("Fail - As all the test steps are passed Test Pass Feedback rating popup not visible so skipping the further test step execution.");
	                    			comments+="Fail- As all the test steps are passed Test Pass Feedback rating popup not visible so skipping the further test step execution .";
	                    			TestUtil.takeScreenShot(this.getClass().getSimpleName(),"FeedbackRatingPopupNotVisible");
	            					return false;
	                    		}
                    		}
	...
	public boolean testPassFeedbackRatingPopUp(String expectedFeedbackRatingPopUpTitle, String Rating, String Feedback) throws InterruptedException
	{
		try
		{
	  ...
	  {
		  t.printStackTrace();					                    				
		  APP_LOGS.debug("Fail - Unable to provide test case feedback . ");						                    			
		  comments+="Fail - Unable to provide test case feedback . ";	
		  return false;
	  }
   }
}

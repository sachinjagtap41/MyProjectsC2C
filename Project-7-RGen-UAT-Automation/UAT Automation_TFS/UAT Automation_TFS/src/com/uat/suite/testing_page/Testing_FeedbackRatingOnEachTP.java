 ...
 * Last Modified on Date:-10th Dec 2014
 * Test Case Description:- Feedback Rating after completion of each Test Pass Activity - 
  A).Verify Go to Feedback
  B).Verify Return Home
  C).Feedback Rating after completion of each Test Pass Activity -Verify in case of Testing Complete, Test Step feedback pop up should not appear.
 */
				...
				try
				{
					...
					*/
				    ...
				    //After performing Configuration Settings Operation Return to the Test Management Page and creation tester and rest of input data 				 
	 				APP_LOGS.debug(this.getClass().getSimpleName()+ " clicking on Test Management tab after configuring Generall Settings for Test Pass : " +testPass[0]+ "" +testPass[1]);
	 				getElement("UAT_testManagement_Id").click();
	 				Thread.sleep(2000);
					...
					if (!createTester(GroupName, PortfolioName, ProjectName, Version, testPass[0], tester.get(0).username, TesterRole1,TesterRole1)) 
					{
						...
						TestUtil.takeScreenShot(this.getClass().getSimpleName(), testPass[0] + " : TesterCreationFailure");				
					...
					{	
						APP_LOGS.debug("Tester Creation Unsuccessfull");		
						comments+="Tester Creation Unsuccessful(Fail)" ;
						TestUtil.takeScreenShot(this.getClass().getSimpleName(), testPass[1] + " : TesterCreationFailure");			
						throw new SkipException("Tester Creation Unsuccessfull");
					}
					...
					if(!customCreateTestCase(GroupName, PortfolioName, ProjectName, Version, testPass[0], TestCaseName))
					{
                    	 ...
                    	 comments+="Fail- Test Case not Created Successfully. ";
     					 TestUtil.takeScreenShot(this.getClass().getSimpleName(), testPass[0] + " : TestCaseCreationUnsuccessfull");
     					 throw new SkipException("Test Case is not created successfully ... So Skipping all tests in Testing Suite");
					}
					
					APP_LOGS.debug(TestCaseName+ " : Test Case Created Successfully in Test Pass : " + testPass[0]);    				
					...
					if(!customCreateTestCase(GroupName, PortfolioName, ProjectName, Version, testPass[1], TestCaseName))
					{
						 APP_LOGS.debug("Test Case is not created successfully");
                    	 comments+="Fail- Test Case not Created Successfully. ";                    	 
                    	 TestUtil.takeScreenShot(this.getClass().getSimpleName(), testPass[1] + " : TestCaseCreationUnsuccessfull");     					  					 
            			 throw new SkipException("Test Case is not created successfully ... So Skipping all tests in Testing Suite");
					}
					APP_LOGS.debug(TestCaseName+ " : Test Case Created Successfully in Test Pass : " + testPass[1]);
    				...
    				{
    					testPass = TestPassName.split(",");
    					if(!searchTestPassAndPerformTesting(ProjectName, Version, testPass[0],TestStepDetails1,TestStepDetails2, TesterRole1, TestStepResult, Rating, Feedback, FeedbackRatingPopupText, "Begin Testing","Go to Feedback"))
    					{
    						fail = true;
    						assertTrue(false);
    					...
    					if(!searchTestPassAndPerformTesting(ProjectName, Version, testPass[1],TestStepDetails1,TestStepDetails2, TesterRole2, TestStepResult, Rating, Feedback, FeedbackRatingPopupText, "Begin Testing","Return to Home"))
    					{
    						fail = true;
    						assertTrue(false);
    						//throw new SkipException("Fail- "+ "Testing for " +testPass[1]+ " with role"+TesterRole2+ " is unsuccessful.");
    					}
    					
    					if(!searchTestPassAndPerformTesting(ProjectName, Version, testPass[0],TestStepDetails1,TestStepDetails2, TesterRole1, TestStepResult, Rating, Feedback, FeedbackRatingPopupText, "Testing Complete",""))
    					{
    						fail = true;
    						assertTrue(false);
    						//throw new SkipException("Fail- "+ "Testing for " +testPass[0]+ " with role"+TesterRole1+ " is unsuccessful.");
	...
	private void configuationSettingFeedbackRating(String ProjectName, String Version , String TestPassName) throws IOException
	{
		...
		*/
		...
		else
		{
			...
			APP_LOGS.debug("Fail -" + feedbackRatingOption1Label + "is not selected as default option in Select Feedback Rating Option . ");

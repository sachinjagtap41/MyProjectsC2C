...
public class VerifySendEmailButtonFunctions extends TestSuiteBase  
{
			...
			if (isLoginSuccess) 
			{
				...
				if (login(versionLead.get(0).username, versionLead.get(0).password, "Version Lead")) 
				{
					...
					if (assertTrue(!(sendEmailToBeginTestingButtonIsDisplayed&&sendTestResultsEmailButtonIsDisplayed))) 
					{
						...
						comments+="'Send Email To Begin Testing' and 'send Test Results Email' Buttons are not Displayed. : Test Case Has been passed";
					}
					else
					{
						fail = true;
					...
					if (assertTrue(sendEmailToBeginTestingButtonIsDisplayed&&sendTestResultsEmailButtonIsDisplayed)) 
					{
						...
						comments+="The email buttons are presnet in presence of test passes(PASS)";
					}
					else
					{
						fail = true;
						
						comments += "The email buttons should have been presnet in presence of test passes but they were not(FAIL)";
					...
					APP_LOGS.debug("Handling Active-x Popup.");
					...
					//	eventfiringdriver.findElement(By.xpath("//span[text()='Ok']")).click();
						...
						throw new SkipException("Cannot create tester");
					}
					Thread.sleep(2000);
				...
				//	gotoTestCasePage();
					...
					if (textFromThePopupAfterSaveButtonOfTCDetails.equals("Test Case added successfully")) 
					{
						...
						throw new SkipException("Exception in creatinfg test case");
					}
					...
					catch (Throwable t) 
					{
						...
						throw new SkipException("Exception in creatinfg test step");
					}
	...
	private void clickOnCancelButtonAndVerifySamePageIsDisplayedAndAlsoAssertPopupText(String Assert_createTesterFirst)
	{
		...
		try 
		{
			...
			if(getObject("TestPasses_testPassHeading").isDisplayed())
	...
	private void clickOnOkButtonAndVerifyTesterPageIsDisplayed()
	{
		try 
		{
			...
			if(getObject("TestPasses_testerDetailsPageHeading").isDisplayed())
			
			APP_LOGS.debug("Click On Ok button. Tester Details Page Is Displayed. Test Case has been Passed");
			else
			{
				fail=true;
				comments+="Tester page was not displayed";
				closeBrowser();
				assertTrue(false);
				APP_LOGS.debug("Click On Ok button. Tester Details Page Is Not Displayed. Test Case has been Failed");
				throw new SkipException("In absence of tester, clicking on Ok of 'Send Email to Begin Testing' button should have redirected to Tester page");
			}
		} 
		catch (Throwable t) 
		{
	...
	private void clickOnOkButtonAndVerifyTestCasePageIsDisplayed(String Assert_createTestCaseNTestStepFirst)
	{
		...
		try 
		{
			...
			if(getObject("TestPasses_testCaseDetailsPageHeading").isDisplayed())
			
			APP_LOGS.debug("Click On Ok button. Test Case Details Page Is Displayed. Test Case has been Passed");
			
			else
			{
				fail=true;
				comments+="Test Case page was not displayed";
				closeBrowser();
				assertTrue(false);
				APP_LOGS.debug("Click On Ok button. Test Case Details Page Is Not Displayed. Test Case has been Failed");
				throw new SkipException("In absence of test case, clicking on Ok of 'Send Email to Begin Testing' button should have redirected to Test Case page");
			}
		} 
		catch (Throwable t) 
		{
